...
public class Decide
{
	public void withdrowMyAccount(Account acc)
	{
		...
		i.e. withdrow()
		and if the method present in the Top class(Account) and these method are 
		overriden in the child class and we are passing the instance of child class then the 
		overridden method of that child class will be executed*/
		...
		else if (acc instanceof DFixedDeposit)
		{
			DFixedDeposit df = (DFixedDeposit)acc;
			acc.withdrow();
		}
		else
		{
			BSavingAccount s = (BSavingAccount)acc;
			acc.withdrow();
		}
	}
}
